export default class Game6 extends Phaser.Scene {
    ...
    preload() {
      ...
      this.load.audio("arcadeweekend3", ["./music/Arcadeweekend3.mp3"]);
      ...
      this.load.spritesheet("invertedsheet", "./public/invertedsheet.png", { frameWidth: 600, frameHeight: 600});
      ...
      this.load.audio("invertedsound1", ["./music/invertedsound1.wav"]);
      this.load.audio("invertedsound2", ["./music/invertedsound2.wav"]);
    ...
    create() {
      ...
      const music = this.sound.add("arcadeweekend3");
      music.loop = true;
   ...
   this.anims.create({
    ...
    this.anims.create({
      key: 'invertedbackg',
      frames: this.anims.generateFrameNumbers('invertedsheet', { start: 0, end: 3 }),
      frameRate: 10,
      repeat: -1
    });
  ...
  update() {
    ...
    //  this.scene.restart();
    //}
    if (this.cursor.left.isDown && !this.activateinverted) {
      this.player.setVelocityX(-this.speed);
      this.player.anims.play('left', true);
    } else if (this.cursor.right.isDown && !this.activateinverted) {
      this.player.setVelocityX(this.speed);
      this.player.anims.play('right', true);
    } else if (!this.cursor.right.isDown && !this.cursor.left.isDown){
      this.player.setVelocityX(0);
      this.player.anims.play('turn', true);
    } else if (this.cursor.left.isDown && this.activateinverted) {
        this.player.setVelocityX(this.speed);
        this.player.anims.play('right', true);
      } else if (this.cursor.right.isDown && this.activateinverted) {
        this.player.setVelocityX(-this.speed);
    ...
    if (this.cursor.up.isDown && this.player.body.touching.down && !this.activateinverted) {
      this.player.setVelocityY(-800);  
    } else if (this.player.body.touching.down && this.cursor.down.isDown && this.activateinverted) {
        this.player.setVelocityY(-800);
      } 
    if (!this.player.body.touching.down && this.cursor.left.isDown && !this.activateinverted) {
      this.player.anims.play("jumpl", true)
    } else if (!this.player.body.touching.down && this.cursor.right.isDown && !this.activateinverted) {
      this.player.anims.play("jumpr", true)
    } else if (!this.player.body.touching.down && this.cursor.left.isDown && this.activateinverted) {
        this.player.anims.play("jumpr", true)
      } else if (!this.player.body.touching.down && this.cursor.right.isDown && this.activateinverted) {
        this.player.anims.play("jumpl", true)
      }
    if (!this.player.body.touching.down && this.cursor.down.isDown && this.activatedown && !this.activateinverted) {
      this.player.setVelocityY(1000);
    } 
    if (this.cursor.up.isDown && !this.player.body.touching.down && this.activateinverted && this.activatedown) {
      this.player.setVelocityY(1000);
    }
    ...
    onSecond2() {
      ...
      recolectable2.setScale(0.5);
      recolectable2.setGravity(0, -1000);
    }

    activateinvert(){
      this.activateinverted = true
      this.invertedsheet = this.physics.add.sprite(500, 300, "invertedsheet")
      this.invertedsheet.anims.play('invertedbackg');
      this.invertedsheet.body.setAllowGravity(false);
      const music = this.sound.add("invertedsound1");
       music.play(); 
      this.time.delayedCall(400, () => {
        this.invertedsheet.destroy()
      })
    ...
    handlerTimer() {
      ...
      if (this.timer <= 0) {
        this.sound.get("arcadeweekend3").stop();
        this.sound.get("arcadeweekend3").destroy();
        this.sound.pauseOnBlur = false;
        ...
        clearTimeout(this.inverted)
       this.scene.start("Gameover6", { score: this.score });  
      }
    }
